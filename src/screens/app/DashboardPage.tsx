...
import { Siren, HandHeart, MapPin, Zap, Activity, Navigation, Stethoscope, CalendarCheck, FolderHeart, Building2 } from 'lucide-react';
...
export const DashboardPage = () => {
  ...
  return (
    <div className="min-h-full bg-[#0a0b0f] dark:bg-[#0a0b0f] flex flex-col px-4 pt-8 pb-4 max-w-lg mx-auto w-full space-y-4">
      ...
      {/* Hospital / Care quick links */}
      <div className="pt-1">
        <div className="flex items-center justify-between mb-2">
          <p className="text-[10px] text-white/35 font-bold uppercase tracking-widest">Hospital & Care</p>
          <Link to="/app/care" className="text-[10px] text-white/50 hover:text-white font-semibold">See all →</Link>
        </div>
        <div className="grid grid-cols-4 gap-2">
          <CareLink to="/app/care" icon={<Stethoscope className="h-4 w-4 text-sky-300" />} label="Find Doctor" />
          <CareLink to="/app/appointments" icon={<CalendarCheck className="h-4 w-4 text-emerald-300" />} label="Appts" />
          <CareLink to="/app/vault" icon={<FolderHeart className="h-4 w-4 text-pink-300" />} label="Vault" />
          <CareLink to="/hospital" icon={<Building2 className="h-4 w-4 text-amber-300" />} label="Hospitals" />
        </div>
      </div>
    </div>
  );
...
const CareLink = ({ to, icon, label }: { to: string; icon: React.ReactNode; label: string }) => (
  <Link
    to={to}
  ...
  >
    {icon}
    <span className="text-[9px] text-white/60 font-semibold">{label}</span>
  </Link>
);
