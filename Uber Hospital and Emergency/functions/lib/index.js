...
function demoAssistantReply(mode, message) {
    if (mode === 'book')
        return `Demo booking assistant: I understood “${message}”. Next I’ll ask: specialty, hospital, date/time.`;
    if (mode === 'meds')
        return `Demo medicine assistant: I understood “${message}”. In production I’ll fetch OpenFDA then summarize safely.`;
    if (mode === 'sos')
        return `Demo SOS coach: If this is an emergency, call your local emergency number. I can guide first-aid steps next.`;
...
function buildAssistantPrompt(mode, message, context) {
    const base = 'You are ResQMed Assistant. Be concise, safety-first, and non-diagnostic. If user mentions emergency symptoms, recommend SOS / emergency services.';
    const modeGuidance = mode === 'book'
