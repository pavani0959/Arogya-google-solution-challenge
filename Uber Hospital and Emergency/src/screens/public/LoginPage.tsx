...
export const LoginPage = () => {
  ...
  const handleVerify = async () => {
    ...
    try {
      if (isDemoMode) {
        await import('../../auth/authActions').then(m => m.loginWithPhone(digits, otp.join('')));
        const profile = await getUserProfile('demo-123');
        setBusy(false);
        if (profile) return nav(redirectPath);
        return nav('/signup', { state: { uid: 'demo-123', phone: digits, redirect: redirectPath } });
      }
      ...
      if (profile) {
        ...
        nav('/signup', { state: { uid: result.user.uid, phone: digits, redirect: redirectPath } });
      }
    } catch (e: any) {
