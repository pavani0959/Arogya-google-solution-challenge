          ...
          <motion.div key="active" initial={{ opacity: 0 }} animate={{ opacity: 1 }} exit={{ opacity: 0 }}
            ...
            <div className="px-5 pb-6 space-y-4 flex-1">
              ...
              {assignments.length > 0 && (
                <div className="rounded-3xl border border-white/[0.06] bg-[#13141a] p-4 space-y-3">
                  <div className="text-[10px] font-bold text-white/30 uppercase tracking-widest">Active Helpers ({assignments.length})</div>
                  {assignments.map((a) => (
                    <div key={a.id} className="flex items-center justify-between rounded-2xl border border-blue-500/15 bg-blue-500/5 px-3 py-2">
                      <div className="text-xs font-bold text-blue-200">{a.helperId.slice(0, 8)}…</div>
                      <div className="text-[10px] text-white/40">{a.distanceMeters ? `${Math.round(a.distanceMeters / 100) * 100}m away` : 'En route'}</div>
                    </div>
                  ))}
                </div>
              )}
